...
// FIXME: display error if user isn't logged in
const EditProfile = () => {
    ...
    const changePassword = () => {
        if (formData.password == "" || formData.password.length < 8) {
            toast.error("Please provide password of atleast 8 characters.")
            return;
        }
        ...
        axios.put("/users/update-password", { ...params })
            .then(response => {
    ...
    const editProfile = () => {
        ...
        if (formData.nationality == "") {
            toast.error("Please provide nationality.")
            return;
        }
        ...
        axios.put("/users/update-profile", { ...params })
            .then(response => {
                //get updated user details
                axios.get("/users//fetch-details/"+user.user_id)
                    .then(response => {
        ...
        <div class="flex flex-col gap-8 px-4 py-6 sm:px-8">
            <div class="flex-1 items-center text-center ">
                ...
                <div class="text-gray-700 p-5">
                    Get to explore your hostels and meet other travellers. Edit your profile below.
                </div>
                ...
                <form id="editProfileForm" action="#" method="post">
                    ...
                    <button type="button" onClick={editProfile} class="min-w-[300px] bg-sun-300 text-black shadow-md font-bold p-3 rounded-full hover:bg-sun-500 focus:outline-none focus:shadow-outline-blue">Save Changes</button>
                </form>
                <div
                    class="text-3xl whitespace-nowrap text-slate-800 sm:text-xl md:text-2xl  font-bold pt-10"
                    style={{ "line-height": "1" }}
                >
                    Edit Password
                </div>
			...
			c0-1.653,1.343-2.998,2.994-2.998c1.652,0,2.996,1.344,2.996,2.998v1.46h0.152c0.617,0,1.125,0.506,1.125,1.125V27.625z"/>
                ...
                <button type="button" onClick={changePassword} class="min-w-[300px] bg-sun-300 text-black shadow-md font-bold p-3 rounded-full hover:bg-sun-500 focus:outline-none focus:shadow-outline-blue">Change Password</button>
